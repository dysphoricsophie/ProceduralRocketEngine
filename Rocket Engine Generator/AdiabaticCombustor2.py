...
def prop_3_exh(productsData):
    ...
    indexA = -1; ProA = str(productsData[0][0]); indexB = -1; indexC = -1
    ProB = str(productsData[0][1]); ProC = str(productsData[0][2]); heado = []; head = []
    for j in range(0, len(Totality)):
        heado.append(Totality[j][0])
    for x in heado:
        head.append(x.split("(")[1].strip(")"))
    ...
    for x in range(1, len(Totality[0])):
        try:
            ...
            Hp = ExpoA * (float(Exh_A[2]) + float(EnthA)) + ExpoB * (float(Exh_B[2]) + float(EnthB)) + ExpoC * (float(Exh_C[2]) + float(EnthC))
            Hp_List.append(float(Hp))
            Hp_Temp.append(float(Totality[0][x + 2]))
        except:
            pass
...
def calculate(reaction):
    ...
    if len(productsData[1]) == 1:
        res = prop_1_exh(productsData)
    elif len(productsData[1]) == 2:
        res = prop_2_exh(productsData)
    elif len(productsData[1]) == 3:
        res = prop_3_exh(productsData)
